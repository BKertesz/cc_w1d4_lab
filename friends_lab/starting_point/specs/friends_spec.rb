...
class TestFriends < MiniTest::Test
  ...
  def test_getting_favorite_tv_show()
    assert_equal("Baywatch",get_favorite_show(@person2))
  end
  ...
  def test_add_friend()
    assert_equal(2,add_friend(@person2,"Darren").length)
  end

  # 5. For a given person, remove a specific name from their list of friends
  # (hint: Same as above, testing for the length of the array should be sufficient)
  def test_remove_friend()
    assert_equal(2,remove_friend(@person4,"Rick").length)
  end
  ...
  def test_loan_to_friend_lender()
    loan_to_friend(@person5,@person2,50)
    assert_equal(50, @person5[:monies])

  end

  def test_loan_to_friend_lendee()
    loan_to_friend(@person5,@person2,50)
    assert_equal(52, @person2[:monies])
    # assert_equal(52, loan_to_friend(@person5,@person2,50))
  end
  ...
  def test_everyones_favourite_food()
    favourites = ["charcuterie","soup","bread","ratatouille", "stew","spaghetti","spinach"]
    assert_equal(favourites, everyones_favourite_food(@people))
  end

  # 9. Find people with no friends
  def test_losers_with_no_friends()
    array = [@person5]
    assert_equal(array, losers_with_no_friends(@people))
  end
